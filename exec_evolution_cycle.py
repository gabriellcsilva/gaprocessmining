...
sublog_evc_keys = np.random.choice(list(full_logfabio2m.keys()), 5, False)  # Choosing an amount of traces
...
for count in range(1):
    ...
    for conf in experiments5:
        ...
        max_len_trace = max([len(foo) for foo in sublog_evc.values()]) * 2
        set_quant = len(sublog_evc) * 2
        exec_id = 'evc-'+'completude-' + str(conf['comp']) + '-precisao-' + str(conf['prec']) + '-crosspoint-' + str(conf['crosspoint']) + '-mutac-' + str(conf['mutac']) + '-tax_cross-' + str(conf['tax_cross']) + '-pop_ex-' + str(conf['pop_ex']) + '-elit-' + str(conf['elitism']) + 'dir_mut' + str(conf['muta_dir']) + 'pc0exec' + str(count)
        # exec_id = 'lol1'

        result = evol.evolution_cycle(alpha_logfabio2m, sublog_evc, size_pop, pop_exchange, max_generations, weights_fit, crossover_setup, mutation_setup, selection_setup, elitism, max_len_trace, set_quant, rpdict_logfabio2m_full, exec_id)
