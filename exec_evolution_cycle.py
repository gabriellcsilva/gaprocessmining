...
sublog_evc = {key: ETM_Configuration1[key] for key in sublog_evc_keys}  # Remounting the traces dict
print(sublog_evc)
...
for count in range(5):
    ...
    for conf in experiments5:
        ...
        exec_id = 'ETM_Configuration1-'+str(size_pop)+'-GEN-'+str(max_generations)+'evc1-30randomtraces'+'completude-' + str(conf['comp']) + '-precisao-' + str(conf['prec']) + '-crosspoint-' + str(conf['crosspoint']) + '-mutac-' + str(conf['mutac']) + '-tax_cross-' + str(conf['tax_cross']) + '-pop_ex-' + str(conf['pop_ex']) + '-elit-' + str(conf['elitism']) + 'dir_mut' + str(conf['muta_dir']) + 'pc0exec' + str(count)
        # exec_id = 'lol1'
        ...
        with open("ETM_Configuration1-output.csv", "a", newline='') as csvfile:
