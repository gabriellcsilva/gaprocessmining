...
sublog_evc_keys = np.random.choice(list(full_logfabio2m.keys()), 35, False)  # Choosing an amount of traces
...
for count in range(1):
    ...
    for conf in experiments5:
        ...
        exec_id = 'evc-10randomtraces'+'completude-' + str(conf['comp']) + '-precisao-' + str(conf['prec']) + '-crosspoint-' + str(conf['crosspoint']) + '-mutac-' + str(conf['mutac']) + '-tax_cross-' + str(conf['tax_cross']) + '-pop_ex-' + str(conf['pop_ex']) + '-elit-' + str(conf['elitism']) + 'dir_mut' + str(conf['muta_dir']) + 'pc0exec' + str(count)
        # exec_id = 'lol1'
