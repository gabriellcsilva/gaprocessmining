stmt = '''import evolution_cycle as evol
...
for count in range(5):
    ...
    for conf in experiments5:
        ...
        max_len_trace = max([len(foo) for foo in logfabio2m.values()]) * 4
        set_quant = len(logfabio2m) * 2
        exec_id = 'sigmoid_weightest-'+'completude-' + str(conf['comp']) + '-precisao-' + str(conf['prec']) + '-crosspoint-' + str(conf['crosspoint']) + '-mutac-' + str(conf['mutac']) + '-tax_cross-' + str(conf['tax_cross']) + '-pop_ex-' + str(conf['pop_ex']) + '-elit-' + str(conf['elitism']) + 'dir_mut' + str(conf['muta_dir']) + 'pc0exec' + str(count)
        # exec_id = 'lol1'
        #todo i need to compare the reference pos set with the individual

        result = evol.evolution_cycle(alpha_logfabio2m, logfabio2m, size_pop, pop_exchange, max_generations, weights_fit, crossover_setup, mutation_setup, selection_setup, elitism, max_len_trace, set_quant, reference_pos_dict_logfabio2m, exec_id)
        ...
        with open("outputevctest.csv", "a", newline='') as csvfile:
