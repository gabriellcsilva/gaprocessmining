...
def evolCicle(tamPop, amt_gens, mut, cross, elite, alfa, logs, execID):

    alfabetoTarefas = alfa
    ...
    elite_rate = elite
    max_generations = amt_gens
    best_fitness = []
    vetor_fitness = []
    set_quant = len(logTraces) * 5

    # For saving the image file
    exec_id_name = str(tamPop) + '-' + str(
        max_generations) + '-' +  str(mutation_rate) + '-' + str(crossover_rate) + '-' + str(elite_rate) + '-exec' + str(execID) + '.png'
    ...
    fields = [tamPop, amt_gens, mut, cross, elite, execID, best_fitness[-1][0], best_fitness[-1][1], max(v_max), sorted_best[-1]]
